var output = function (input) {
    ...
    input.draw = function () {
        input.clear();
        twinkleStars(bigStars);

        if (shootingStars[0] == null) {
                shootingStars[0] = new ShootingStar(
                    2, 
                    Math.floor(Math.random() * width), 
                    Math.floor(Math.random() * height), 
                );
        } else {
            if (currentShootingStarAge < 300) {
                shootingStars[0].move();
                currentShootingStarAge++;
                drawStars(shootingStars);
            } else {
                shootingStars[0] = null;
                currentShootingStarAge = 0;
            }
        }
...
var starsDisplay = new p5(output, "stars");
